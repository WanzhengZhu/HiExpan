...
__description__: Python Implementation of SetExpan algorithm (used as Width Expansion in HiExpan)
"""
import random
import math
import time
...
def setExpan(seedEidsWithConfidence, negativeSeedEids, eid2patterns, pattern2eids, eidAndPattern2strength,
    ...
    while iters < MAX_ITER_SET:
        ...
        if FLAGS_DEBUG:
            print("[INFO] Number of candidate entities matched by skipgrams = %s" % len(combinedSgSimByCandidateEid.keys()))
            if use_type:
